...
class MLP_Classifier(object):
  ...
  def evaluate_algorithm(self, dataset, target, algorithm, n_epochs, *args):
	accuracy_max = 0.0
	  ...
	  for epoch in range(n_epochs):
		...
		model = TruncatedSVD(n_components=self.best_dimension)
		model.fit(X_train)
		model.fit(X_validate)
		select_X_train = model.transform(X_train)
		select_X_validate = model.transform(X_validate)
		print('After reduction: (%d,%d)' %select_X_train.shape)
		print('After reduction: (%d,%d)' %select_X_validate.shape)

		select_X_train = rescale_dataset(select_X_train)
                select_X_validate = rescale_dataset(select_X_validate)

                y_pred_validate = algorithm(select_X_train, y_train, select_X_validate, *args)
                accuracy = accuracy_metric(y_validate, y_pred_validate)
		accuracy_avg += accuracy
                print("Validate accuracy %.3f\n" % (accuracy))
	  accuracy_avg = accuracy_avg/n_epochs
	  print('Accuracy avg %.3f' % accuracy_avg)
          if (accuracy_avg > accuracy_max):
          	accuracy_max = accuracy_avg
		self.best_hidden = x
		if self.action == 'train':
                	np.savetxt('weights_wi.out2', self.wi, fmt='%.5f')
                	np.savetxt('weights_wo.out2', self.wo, fmt='%.5f')
	print('Accuracy max %.3f at hidden layer %d' % (accuracy_max, self.best_hidden))
  ...
  def __init__(self, n_input, n_hidden, n_output):
	...
	self.best_hidden = self.hidden
	self.best_dimension = 40
	self.input = 40
	self.action = 'show'
  ...
  def init_weights(self, test=False):
	...
	if test == True or self.action == 'show':
 		self.wi = np.loadtxt('weights_wi.out2')
		self.wo = np.loadtxt('weights_wo.out2')
		self.hidden = self.best_hidden
  ...
  def backPropagate(self, targets, N):
    ...
    :param l: regularization controll
...
def preprocessing(df_data, cols_to_remove, categorical_features, n_components):
...
def demo():
  ...
  action = 'show' # default to show result
  ...
  print('Training dataset: (%d,%d)' %df_data.shape)
  ...
  print("New shape of the training set: (%d,%d)" %df_data.shape)
  print("The removed columns are:")
  for column in cols_to_remove:
  ...
  n_hidden = 40
  start = time.time()
  NN = MLP_Classifier(X.shape[1], n_hidden, 2)
  NN.action = action
  NN.init_weights()
  if action == 'train':
  	NN.evaluate_algorithm(X, y, NN.back_propagation, n_epochs)
  else:
	X_train, X_validate, y_train, y_validate \
                         = train_test_split(X, y, test_size=0.2, random_state=np.random.randint(1,128))
        # Dimensionality Reduction
        model = TruncatedSVD(n_components=NN.best_dimension)
        model.fit(X_train)
        select_X_train = model.transform(X_train)
        print('After reduction: (%d,%d)' %select_X_train.shape)
  ...
  if action == 'test':
	  ...
	  X_test, y_test = preprocessing(df_test, cols_to_remove, categorical_features, NN.best_dimension)
	  # test set accuracy
